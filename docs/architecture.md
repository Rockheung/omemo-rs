...
* Stage 4 — `omemo-pep` (✅): PEP publish/fetch for own device list and
  ...
  Prosody.
* Stage 5 — Group OMEMO (⏳): extends Stage 4 to MUC rooms
  (XEP-0384 §6). Adds occupant-JID resolution and bundle-fetch
  backpressure.
...
### `omemo-pep` ✅
XMPP integration on top of `tokio-xmpp 5` (MPL-2.0, ADR-007). Three
internal modules:
...
Plaintext localhost connect via `connect_plaintext` for tests; the
production StartTLS path (real XMPP servers) is queued in TODO.md.
