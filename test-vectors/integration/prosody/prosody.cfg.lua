...
-- Stub VirtualHosts auto-generated from the cert's SAN list (one
-- per operator-reachable hostname: machine name, mDNS-style local
-- alias, LAN IP, …). Prosody's TLS SNI callback rejects any
-- hostname that doesn't match a configured VirtualHost (or
-- Component), so without these stubs `wss://gpus.lcl:5281/...`
-- would die with `unrecognized_name` even though the cert SAN
-- covers `gpus.lcl`. The stubs are pure SNI-matching shells:
-- accounts, modules, and PEP nodes all stay on the `localhost`
-- VirtualHost; clients send `<stream:stream to='localhost'>`
-- after the handshake so XMPP-level routing is unaffected.
--
-- Generated by `test-vectors/integration/tls/gen-cert.sh` —
-- regenerated every time the cert is regenerated, so the SAN
-- list and the VirtualHost list stay in lockstep.
--
-- The Include MUST come before `VirtualHost "localhost"` because
-- Prosody's config grammar treats every line after a VirtualHost
-- declaration as belonging to that host until the next top-level
-- VirtualHost / Component / `Include` directive.
Include "/etc/prosody/extra-vhosts.cfg.lua"
