...
# Emit a Prosody sidecar config that registers a stub VirtualHost
# for every DNS name in the cert SAN. Prosody's TLS SNI callback
# is strict — an SNI for any hostname not in `VirtualHost` (or
# `Component`) declarations triggers TLS alert
# `unrecognized_name` and the connection is dropped, even though
# the cert itself covers the name. The stubs exist purely to make
# Prosody answer `e2e_alice@localhost` over SNI=`gpus.lcl` —
# accounts and modules stay on the `localhost` VirtualHost; the
# stream `to=` attribute does the XMPP-level routing
# independently of TLS SNI.
out_cfg="$(realpath ..)/prosody/extra-vhosts.cfg.lua"
{
    echo '-- Auto-generated by `tls/gen-cert.sh`. Do not edit.'
    echo '-- Stub VirtualHosts so Prosody accepts any SNI in the cert SAN.'
    for h in "${SAN_HOSTS[@]}"; do
        # Skip entries already configured by hand or impossible to
        # serve as a JID domain (wildcards, container DNS).
        case "$h" in
            "localhost") continue ;;
            "*."*)       continue ;;
            "omemo-rs-prosody"|"omemo-rs-converse") continue ;;
        esac
