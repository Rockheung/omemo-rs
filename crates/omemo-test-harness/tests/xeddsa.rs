...
fn replay_xeddsa_fixtures() {
    ...
    for c in &fixture.cases {
        ...
        f.check(
            &c.label,
            "seed_to_priv",
            xed::seed_to_priv(&ed_seed).to_vec(),
            &c.seed_to_priv_hex,
        );
        f.check(
            &c.label,
            "seed_to_ed25519_pub",
            xed::seed_to_ed25519_pub(&ed_seed).to_vec(),
            &c.seed_to_ed_pub_hex,
        );
        f.check(
            &c.label,
            "priv_to_curve25519_pub",
            xed::priv_to_curve25519_pub(&priv_).to_vec(),
            &c.priv_to_curve_pub_hex,
        );
        f.check(
            &c.label,
            "priv_to_ed25519_pub",
            xed::priv_to_ed25519_pub(&priv_).to_vec(),
            &c.priv_to_ed_pub_hex,
        );
        f.check(
            &c.label,
            "priv_force_sign(false)",
            xed::priv_force_sign(&priv_, false).to_vec(),
            &c.priv_force_sign_false_hex,
        );
        f.check(
            &c.label,
            "priv_force_sign(true)",
            xed::priv_force_sign(&priv_, true).to_vec(),
            &c.priv_force_sign_true_hex,
        );
        ...
        f.check(
            &c.label,
            "curve→ed (sign=0)",
            xed::curve25519_pub_to_ed25519_pub(&curve_pub, false).to_vec(),
            &c.curve_to_ed_sign0_hex,
        );
        f.check(
            &c.label,
            "curve→ed (sign=1)",
            xed::curve25519_pub_to_ed25519_pub(&curve_pub, true).to_vec(),
            &c.curve_to_ed_sign1_hex,
        );
        ...
        f.check(
            &c.label,
            "ed25519_seed_sign",
            xed::ed25519_seed_sign(&ed_seed, &msg).to_vec(),
            &c.ed25519_seed_sig_hex,
        );

        f.check(
            &c.label,
            "ed25519_priv_sign (XEdDSA)",
            xed::ed25519_priv_sign(&priv_, &msg, &nonce).to_vec(),
            &c.xeddsa_priv_sig_hex,
        );

        let x25519 = xed::x25519(&priv_, &peer_curve_pub).expect("x25519");
        f.check(&c.label, "x25519", x25519.to_vec(), &c.x25519_shared_hex);
    ...
    if !f.0.is_empty() {
        panic!(
            "xeddsa replay failures ({} of {} primitives × {} cases):\n{}",
            f.0.len(),
            13,
            fixture.cases.len(),
            f.0.join("\n")
